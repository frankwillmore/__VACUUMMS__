...
void samplePoints()
{
  ...
  {
    px[number_of_points] = box_x * rnd2();
    py[number_of_points] = box_y * rnd2();
    pz[number_of_points] = box_z * rnd2();

    // check inclusion
    okay_flag = FALSE;
    for (j=0; j<number_of_cavities; j++)
    {
      dx = x[j] - px[number_of_points];
      dy = y[j] - py[number_of_points];
      dz = z[j] - pz[number_of_points];
