...
//  getDoubleParam("-step_size_factor", &step_size_factor);
  ...
  getDoubleParam("-time_step", &time_step);
  ...
  {
    printf("\n");
    printf("usage:  -seed [ 123450 ]\n");
    printf("        -randomize\n");
    printf("        -box [ 6.0 6.0 6.0 ]\n");
    printf("        -verlet_cutoff [ 100.0 ]\n");
    printf("        -test_diameter [ 1.0 ] in A\n");
    printf("        -test_epsilon [ 1.0 ] in K\n");
    printf("        -T [ 298.0 ]\n");
    printf("        -trace \n");
    printf("        -n [ 1 ]\n");
    printf("        -n_steps [ 1024 ]\n");
    printf("        -test_mass (note kg/mol)[ 1.0 ]\n");
    printf("        -time_step (s)[ 0.0 ]\n");
//    printf("        -step_size_factor [ 1.0 ] \n");
    printf("\nAll temperatures are in K, as are energies (epsilon)\n");
    exit(0);
  }
...
//  beta=1/(kB*T);
  ...
  while (number_of_samples-- > 0)
  {
    ...
    for (successes=0; successes<n_steps; attempts++)
    {
      if (trace) printf ("%lf\t%lf\t%lf\n", test_x, test_y, test_z);
      attemptRandomStep();
...
attemptRandomStep()
{
  ...
  double step_size;
  double i_hat, j_hat, k_hat;
  double hat_size;

// begin construction

  kinetic_energy = -log(rnd())*8.314*T; // in J/mol
  velocity = sqrt(2.0*kinetic_energy/test_mass); // in m/s
  step_size = velocity * time_step * 1.0e+10; // to Angstroms

  i_hat=rnd()-.5;
  j_hat=rnd()-.5;
  k_hat=rnd()-.5;

  hat_size=sqrt(i_hat*i_hat + j_hat*j_hat + k_hat*k_hat);

  step_x = step_size*i_hat/hat_size;
  step_y = step_size*j_hat/hat_size;
  step_z = step_size*k_hat/hat_size;
...
// end construction
  ...
  if ((new_energy < energy) || (rnd() < exp(-(new_energy-energy)/T)))
  {
    successes++;
    return;
  }
...
generateTestPoint()
{
  ...
  while (rnd() > exp(calculateEnergy()/T)) 
  {
    test_x = test_x0 = rnd() * box_x;
    test_y = test_y0 = rnd() * box_y;
    test_z = test_z0 = rnd() * box_z;
